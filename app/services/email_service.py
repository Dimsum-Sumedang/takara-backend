    ...
    async def _dispatch_email(self, to_email: str, subject: str, html_content: str):
        ...
        if self.settings.BREVO_API_KEY:
            try:
                ...
                def _brevo_request():
                    req_data = json.dumps({
                        "sender": {"name": "Takara", "email": sender_email},
                        "to": [{"email": to_email}],
                        "subject": subject,
        ...
        if self.settings.RESEND_API_KEY:
            try:
                resend_from = self.settings.SMTP_FROM if "resend.dev" in (self.settings.SMTP_FROM or "") else "Takara <onboarding@resend.dev>"
    ...
    async def send_otp_email(self, to_email: str, otp_code: str):
        ...
        <html>
            <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                <h2 style="color: #333;">Welcome to Takara!</h2>
        ...
        """
        await self._dispatch_email(to_email, "Takara - Your Verification Code", html_content)

    async def send_reset_password_email(self, to_email: str, otp_code: str):
        html_content = f"""
        <html>
            <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                <h2 style="color: #333;">Reset Your Password - Takara</h2>
        ...
        """
        await self._dispatch_email(to_email, "Takara - Password Reset Code", html_content)
